...
class EmployeeDetails(models.Model):
    ...
    profile = models.ImageField(upload_to='img/', blank=True, null=True)

    def __str__(self):
        return self.empcode

class EmployeeEduction(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
...
class EmpAchievement(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True)
    guest_lec = models.CharField(max_length=200)
    project_name = models.CharField(max_length=200)
    upload = models.FileField(upload_to="file/", null=True,default="")

    def __str__(self):
        return self.project_name

class Emp_Paper(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True)
...
class Emp_Journals(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True)
    option = models.CharField(max_length=100)
    upload = models.FileField(upload_to="file/", null=True, default="")
    def __str__(self):
